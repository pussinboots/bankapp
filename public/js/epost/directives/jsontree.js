angular.module('jsontree', []).directive('jsonTree', ['$compile', '$parse', function ($compile, $parse) {
    return {
        terminal: true,
        replace: false,
        restrict: 'A',
...
angular.module('debugbox', []).directive('debugbox', ['$compile', '$parse', function ($compile, $parse) {
    return {
        transclude: true,
        replace: false,
        restrict: 'A',
        scope: {
        	data:'@debugData',
        	collapsedDebug:'@expand'
        },
        template: '<div class="debugbox">' +
			        '<button ng-click="collapsedDebug=!collapsedDebug">+</button>' +
			        	'<div ng-show="collapsedDebug">' +
			        	'<div class="well span6">' +
				        '<div json-tree="{{data}}"></div>' +
				        '<div>{{data}}</div>' +
				    '</div>' +
			      '</div></div>',
        link: function (scope, element, attrs) {
