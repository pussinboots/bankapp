...
class SparKassenClient() {
  ...
  def parseStockOverview(html: String) = {
    ...
    def dataFunc(data:Seq[String], actions:Map[String, TagNode]) = (normalizeName(data(1)) -> Account(normalizeName(data(1)), 1.toLong, data(4).split(" ")(0).replaceAll("\\.", "").replaceAll(",", "\\.").toDouble, actions))
